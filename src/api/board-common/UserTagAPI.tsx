...
export const checkFavoriteTag = async (tagName: string): Promise<boolean> => {
  const response = await API.get<boolean>(`/api/v1/favorites/tags/${tagName}`);
  return response.data;
};

// 회원 관심 태그 추가 (POST)
export const addFavoriteTag = async (tagName: string): Promise<void> => {
  await API.post(`/api/v1/favorites/tags/${tagName}`);
};

// 회원 관심 태그 삭제 (DELETE)
export const removeFavoriteTag = async (tagName: string): Promise<void> => {
  await API.delete(`/api/v1/favorites/tags/${tagName}`);
};
...
export const getFavoriteTags = async (): Promise<FavoriteTagsResponse> => {
  const response = await API.get<FavoriteTagsResponse>("/api/v1/favorites/tags");
  return response.data;
};
