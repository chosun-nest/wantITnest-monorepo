...
export default function Home() {
  ...
  return (
    <S.GridContainer $navbarHeight={navbarHeight} $isMobile={isMobile}>
      <S.GridItem $row="1" $isMobile={isMobile} $col="1" $rowSpan="2">
        {!isLoggedIn ? (
          <GuestCard />
        ) : (
          <S.ProfileContainer>
      ...
      {isLoggedIn ? (
        <S.GridItem $row="4" $col="1" $colSpan="3" $isMobile={isMobile}>
          <S.ItemTitle>내 핀</S.ItemTitle>
          <MyPin title="" editable />
        </S.GridItem>
      ) : null}
      <S.GridItem $row="1" $isMobile={isMobile} $col="2" $colSpan="2">
        <S.ItemTitle>
          관심분야 게시판<Link to="/interests-board">+</Link>
        </S.ItemTitle>
        <HomeInterest />
      </S.GridItem>
      <S.GridItem $row="2" $isMobile={isMobile} $col="2" $colSpan="2">
        <S.ItemTitle>
          학사 공지<Link to="/notice-board">+</Link>
        </S.ItemTitle>
        <HomeNotice />
      </S.GridItem>{" "}
      <S.GridItem $row="3" $isMobile={isMobile} $col="1" $colSpan="1">
        <S.ItemTitle>행사</S.ItemTitle>
      </S.GridItem>
      <S.GridItem $row="3" $isMobile={isMobile} $col="2" $colSpan="2">
        <S.ItemTitle>
